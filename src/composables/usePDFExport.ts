...
export function usePDFExport() {
  ...
  const exportDashboardToPDF = async (initialData?: DashboardData, options: ExportOptions = {}) => {
    ...
    try {
      ...
      pdf.setProperties({
        title: 'D2 Nutrition Vibes - Dashboard Report',
        ...
        author: 'D2 Nutrition Vibes',
        creator: 'D2 Nutrition Vibes Application'
      })
  ...
  const addHeader = async (pdf: jsPDF, data: DashboardData, yPos: number): Promise<number> => {
    ...
    pdf.text('D2 Nutrition Vibes - Dashboard Report', 20, yPos)
    yPos += 15
  ...
  const addFooter = (pdf: jsPDF) => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Footer text
      pdf.text('Generated by D2 Nutrition Vibes', 20, 285)
